...
func authorizationPage(w http.ResponseWriter, r *http.Request) {
	...
	if token == nil || token.Value == "" {
		http.Redirect(w, r, "/login?original_url="+r.URL.String(), http.StatusFound)
		return
	}

	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	fmt.Fprintf(w, "<form action='/authorize' method='POST'>")
	fmt.Fprintf(w, "<input type='hidden' name='client_id' value='client_id' />")
	fmt.Fprintf(w, "<input type='hidden' name='callback' value='http://localhost:8181/callback' />")
...
func authorize(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response(w, nil, fmt.Errorf("invalid auth_token"), http.StatusUnauthorized)
		return
	}
